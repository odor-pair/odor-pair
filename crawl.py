...
def get_notes(soup):
    note_links = soup.find_all(
        'a',
        {'href': re.compile(r'http://www\.thegoodscentscompany\.com/odor/')})
    return [r.text for r in note_links]


def get_blenders(soup):
    table_section = soup.find(string='Potential Blenders and core components '
                              ).parent.find_next('table')
...
def get_all_basedomains():
    ...
    soup = get_soup(url)
    results = soup.find_all(attrs={
        'href':
        re.compile(r"http://www\.thegoodscentscompany\.com/rawmatex.*")
    })
